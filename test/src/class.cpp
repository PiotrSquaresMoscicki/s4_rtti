...
TEST_CASE( "rtti::Class::size", "[rtti::Class]" ) {
    REQUIRE( 
        static_type<TestClassNotDefaultConstructible>::get()->size() 
        == 
        sizeof(TestClassNotDefaultConstructible) );

    REQUIRE( 
        static_type<TestClassNotCopyAssignable>::get()->size() 
        == 
...
TEST_CASE( "rtti::Class::as_fundamental", "[rtti::Class]" ) {
    REQUIRE( static_type<TestClassNotDefaultConstructible>::get()->as_fundamental().is_err() );
}

//*************************************************************************************************
TEST_CASE( "rtti::Class::as_enum", "[rtti::Class]" ) {
    REQUIRE( static_type<TestClassNotDefaultConstructible>::get()->as_enum().is_err() );
}
...
TEST_CASE( "rtti::Class::is_default_constructible", "[rtti::Class]" ) {
    REQUIRE( static_type<TestClassNotDefaultConstructible>::get()->is_default_constructible() == false );
    REQUIRE( static_type<TestClassNotCopyAssignable>::get()->is_default_constructible() == true );
}

//*************************************************************************************************
TEST_CASE( "rtti::Class::is_copy_constructible", "[rtti::Class]" ) {
    REQUIRE( static_type<TestClassNotCopyConstructible>::get()->is_copy_constructible() == false );
    REQUIRE( static_type<TestClassNotDefaultConstructible>::get()->is_copy_constructible() == true );
}

//*************************************************************************************************
TEST_CASE( "rtti::Class::is_move_constructible", "[rtti::Class]" ) {
    REQUIRE( static_type<TestClassNotMoveConstructible>::get()->is_move_constructible() == false );
    REQUIRE( static_type<TestClassNotDefaultConstructible>::get()->is_move_constructible() == true );
}

//*************************************************************************************************
TEST_CASE( "rtti::Class::is_copy_assignable", "[rtti::Class]" ) {
    REQUIRE( static_type<TestClassNotCopyAssignable>::get()->is_copy_assignable() == false );
    REQUIRE( static_type<TestClassNotDefaultConstructible>::get()->is_copy_assignable() == true );
}

//*************************************************************************************************
TEST_CASE( "rtti::Class::is_move_assignable", "[rtti::Class]" ) {
    REQUIRE( static_type<TestClassNotMoveAssignable>::get()->is_move_assignable() == false );
    REQUIRE( static_type<TestClassNotDefaultConstructible>::get()->is_move_assignable() == true );
}

//*************************************************************************************************
TEST_CASE( "rtti::Class::alloc_construct", "[rtti::Class]" ) {
    Object obj = static_type<TestClassNotMoveAssignable>::get()->alloc_construct().ok();
    REQUIRE( obj.is_valid() == true );
    REQUIRE( obj.type().ok() == static_type<TestClassNotMoveAssignable>::get() );
    REQUIRE( obj.size().ok() == sizeof(TestClassNotMoveAssignable) );
}

//*************************************************************************************************
TEST_CASE( "rtti::Class::alloc_copy_construct", "[rtti::Class]" ) {
    Object src = static_type<TestClassNotMoveAssignable>::get()->alloc_construct().ok();
    src.value_as<TestClassNotMoveAssignable>().ok()->m_int_val = 4;
    Object dst = static_type<TestClassNotMoveAssignable>::get()->alloc_copy_construct(src).ok();
    REQUIRE( dst.is_valid() == true );
    REQUIRE( dst.type().ok() == static_type<TestClassNotMoveAssignable>::get() );
    REQUIRE( dst.size().ok() == sizeof(TestClassNotMoveAssignable) );
    REQUIRE( dst.value_as<TestClassNotMoveAssignable>().ok()->m_int_val == 4 );

    REQUIRE( 
        static_type<TestClassNotMoveAssignable>::get()->alloc_copy_construct(Object()).err() 
        == 
        Type::ErrCopyConstruct::NOT_VALID_SOURCE );

    src = static_type<TestClassNotCopyAssignable>::get()->alloc_construct().ok();
    REQUIRE( 
        static_type<TestClassNotMoveAssignable>::get()->alloc_copy_construct(src).err() 
        == 
...
TEST_CASE( "rtti::Class::alloc_move_construct", "[rtti::Class]" ) {
    ...
    Object dst = static_type<TestClassNotMoveAssignable>::get()->alloc_move_construct(src).ok();
    REQUIRE( dst.is_valid() == true );
    REQUIRE( dst.type().ok() == static_type<TestClassNotMoveAssignable>::get() );
    REQUIRE( dst.size().ok() == sizeof(TestClassNotMoveAssignable) );
    ...
    Object invalid_src;
    REQUIRE( 
        static_type<TestClassNotMoveAssignable>::get()->alloc_move_construct(invalid_src).err() 
        ==
        Type::ErrMoveConstruct::NOT_VALID_SOURCE );

    src = static_type<TestClassNotCopyAssignable>::get()->alloc_construct().ok();
    REQUIRE( 
        static_type<TestClassNotMoveAssignable>::get()->alloc_move_construct(src).err()
        ==
...
TEST_CASE( "rtti::Class::can_construct", "[rtti::Class]" ) {
    ...
    REQUIRE( 
        static_type<TestClassNotDefaultConstructible>::get()->can_construct(src).err() 
        == 
        Type::ErrConstruct::NOT_DEFAULT_CONSTRUCTIBLE );
    
    REQUIRE( 
        static_type<TestClassNotMoveAssignable>::get()->can_construct(src).err() 
        == 
    ...
    REQUIRE( 
        static_type<TestClassNotMoveAssignable>::get()->can_construct(src).is_ok() 
        == 
        true );
...
TEST_CASE( "rtti::Class::construct", "[rtti::Class]" ) {
    ...
    ObjectRef obj_ref = static_type<TestClassNotMoveConstructible>::get()->construct(std::move(buff_ref)).ok();
    REQUIRE( obj_ref.is_valid() == true );
    REQUIRE( obj_ref.type().ok() == static_type<TestClassNotMoveConstructible>::get() );
    ...
    Object obj = static_type<TestClassNotMoveConstructible>::get()->construct(std::move(buff)).ok();
    REQUIRE( obj.is_valid() == true );
    REQUIRE( obj.type().ok() == static_type<TestClassNotMoveConstructible>::get() );
...
TEST_CASE( "rtti::Class::can_copy_construct", "[rtti::Class]" ) {
    ...
    REQUIRE( 
        static_type<TestClassNotCopyConstructible>::get()->can_copy_construct(buff, src).err() 
        == 
        Type::ErrCopyConstruct::NOT_COPY_CONSTRUCTIBLE );

    REQUIRE( 
        static_type<TestClassNotMoveConstructible>::get()->can_copy_construct(buff, src).err() 
        == 
    ...
    src = static_type<TestClassNotCopyConstructible>::get()->alloc_construct().ok();
    REQUIRE( 
        static_type<TestClassNotMoveConstructible>::get()->can_copy_construct(buff, src).err() 
        == 
        Type::ErrCopyConstruct::INCORRECT_SOURCE_TYPE );

    src = static_type<TestClassNotMoveConstructible>::get()->alloc_construct().ok();
    REQUIRE( static_type<TestClassNotMoveConstructible>::get()->can_copy_construct(buff, src).is_ok() == true );
}
...
TEST_CASE( "rtti::Class::copy_construct", "[rtti::Class]" ) {
    ...
    ObjectRef obj_ref = static_type<TestClassNotMoveAssignable>::get()->copy_construct(std::move(buff_ref), src).ok();
    REQUIRE( obj_ref.is_valid() == true );
    REQUIRE( obj_ref.type().ok() == static_type<TestClassNotMoveAssignable>::get() );
    REQUIRE( obj_ref.size().ok() == 30 );
    REQUIRE( obj_ref.value().ok() == reinterpret_cast<void*>(src_array) );
    REQUIRE( obj_ref.value_as<TestClassNotMoveAssignable>().ok()->m_int_val == 57 );

    Buffer buff = Buffer(sizeof(TestClassNotMoveAssignable));
    Object obj = static_type<TestClassNotMoveAssignable>::get()->copy_construct(std::move(buff), src).ok();
    REQUIRE( obj.is_valid() == true );
    REQUIRE( obj.type().ok() == static_type<TestClassNotMoveAssignable>::get() );
    REQUIRE( obj.size().ok() == sizeof(TestClassNotMoveAssignable) );
...
TEST_CASE( "rtti::Class::can_move_construct", "[rtti::Class]" ) {
    ...
    REQUIRE( 
        static_type<TestClassNotMoveConstructible>::get()->can_move_construct(buff, src).err() 
        == 
        Type::ErrMoveConstruct::NOT_MOVE_CONSTRUCTIBLE );
    
    REQUIRE( 
        static_type<TestClassNotCopyConstructible>::get()->can_move_construct(buff, src).err() 
        == 
    ...
    src = static_type<TestClassNotCopyConstructible>::get()->alloc_construct().ok();
    REQUIRE( static_type<TestClassNotCopyConstructible>::get()->can_move_construct(buff, src).is_ok() == true );
}
...
TEST_CASE( "rtti::Class::move_construct", "[rtti::Class]" ) {
    ...
    Object src = static_type<TestClassNotMoveAssignable>::get()->alloc_construct().ok();
    src.value_as<TestClassNotMoveAssignable>().ok()->m_int_val = 57;
    ObjectRef obj_ref = static_type<TestClassNotMoveAssignable>::get()->move_construct(std::move(buff_ref), src).ok();
    REQUIRE( obj_ref.is_valid() == true );
    REQUIRE( obj_ref.type().ok() == static_type<TestClassNotMoveAssignable>::get() );
    REQUIRE( obj_ref.size().ok() == 30 );
    REQUIRE( obj_ref.value().ok() == reinterpret_cast<void*>(src_array) );
    ...
    Buffer buff = Buffer(sizeof(TestClassNotMoveAssignable));
    Object obj = static_type<TestClassNotMoveAssignable>::get()->move_construct(std::move(buff), src).ok();
    REQUIRE( obj.is_valid() == true );
    REQUIRE( obj.type().ok() == static_type<TestClassNotMoveAssignable>::get() );
    REQUIRE( obj.size().ok() == sizeof(TestClassNotMoveAssignable) );
    REQUIRE( obj.value_as<TestClassNotMoveAssignable>().ok()->m_int_val == 57 );
...
TEST_CASE( "rtti::Class::can_destruct", "[rtti::Class]" ) {
    ...
    REQUIRE( 
        static_type<TestClassNotDestructible>::get()->can_destruct(obj).err() 
        == 
        Type::ErrDestruct::NOT_DESTRUCTIBLE );

    REQUIRE( 
        static_type<TestClassNotMoveAssignable>::get()->can_destruct(obj).err() 
        == 
        Type::ErrDestruct::NOT_VALID_OBJECT );
    
    obj = static_type<TestClassNotCopyAssignable>::get()->alloc_construct().ok();
    REQUIRE( 
        static_type<TestClassNotMoveAssignable>::get()->can_destruct(obj).err() 
        == 
        Type::ErrDestruct::INCORRECT_OBJECT_TYPE );
    
    obj = static_type<TestClassNotMoveAssignable>::get()->alloc_construct().ok();
    REQUIRE( static_type<TestClassNotMoveAssignable>::get()->can_destruct(obj).is_ok() == true );
}
...
TEST_CASE( "rtti::Class::dealloc_destruct", "[rtti::Class]" ) {
    ...
    REQUIRE( static_type<TestClassNotMoveAssignable>::get()->dealloc_destruct(std::move(obj)).is_ok() == true );
...
TEST_CASE( "rtti::Class::destruct", "[rtti::Class]" ) {
    char src_array[30];
    BufferRef buff_ref(reinterpret_cast<void*>(src_array), 30);
    ObjectRef obj_ref = static_type<TestClassNotMoveAssignable>::get()->construct(std::move(buff_ref)).ok();
    buff_ref = static_type<TestClassNotMoveAssignable>::get()->destruct(std::move(obj_ref)).ok();
    ...
    Object obj = static_type<TestClassNotMoveAssignable>::get()->alloc_construct().ok();
    const void* obj_value_ptr = obj.value().ok();
    Buffer buff = static_type<TestClassNotMoveAssignable>::get()->destruct(std::move(obj)).ok();
...
TEST_CASE( "rtti::Class::copy_assign", "[rtti::Class]" ) {
    ...
    REQUIRE( 
        static_type<TestClassNotCopyAssignable>::get()->copy_assign(dst, src).err() 
        == 
        Type::ErrCopy::NOT_COPY_ASSIGNABLE);
        
    REQUIRE( 
        static_type<TestClassNotMoveAssignable>::get()->copy_assign(dst, src).err() 
        == 
        Type::ErrCopy::INVALID_DESTINATION_OBJECT);

    dst = static_type<TestClassNotCopyAssignable>::get()->alloc_construct().ok();
    REQUIRE( 
        static_type<TestClassNotMoveAssignable>::get()->copy_assign(dst, src).err() 
        == 
        Type::ErrCopy::INCORRECT_DESTINATION_OBJECT_TYPE );

    dst = static_type<TestClassNotMoveAssignable>::get()->alloc_construct().ok();
    REQUIRE( 
        static_type<TestClassNotMoveAssignable>::get()->copy_assign(dst, src).err() 
        == 
    ...
    src = static_type<TestClassNotMoveAssignable>::get()->alloc_construct().ok();
    src.value_as<TestClassNotMoveAssignable>().ok()->m_int_val = 6565656;
    REQUIRE( static_type<TestClassNotMoveAssignable>::get()->copy_assign(dst, src).is_ok() == true );
...
TEST_CASE( "rtti::Class::move_assign", "[rtti::Class]" ) {
    ...
    REQUIRE( 
        static_type<TestClassNotMoveAssignable>::get()->move_assign(dst, src).err() 
        == 
        Type::ErrMove::NOT_MOVE_ASSIGNABLE);
        
    REQUIRE( 
        static_type<TestClassNotCopyAssignable>::get()->move_assign(dst, src).err() 
        == 
    ...
    REQUIRE( static_type<TestClassNotCopyAssignable>::get()->move_assign(dst, src).is_ok() == true );
