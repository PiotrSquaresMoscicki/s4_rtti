...
TEST_CASE( "rtti::Enum::alloc_construct", "[rtti::Enum]" ) {
    Object obj = static_type<TestEnum1>()->alloc_construct().ok();
...
TEST_CASE( "rtti::Enum::alloc_copy_construct", "[rtti::Enum]" ) {
    Object src = static_type<TestEnum1>()->alloc_construct().ok();
    *src.value_as<TestEnum1>().ok() = TestEnum1::VAL_1;
    Object dst = static_type<TestEnum1>()->alloc_copy_construct(src).ok();
    ...
    REQUIRE( static_type<TestEnum1>()->alloc_copy_construct(Object()).err() == Type::ErrCopyConstruct::NOT_VALID_SOURCE );

    src = static_type<TestEnum2>()->alloc_construct().ok();
    REQUIRE( static_type<TestEnum1>()->alloc_copy_construct(src).err() == Type::ErrCopyConstruct::INCORRECT_SOURCE_TYPE );
}
...
TEST_CASE( "rtti::Enum::alloc_move_construct", "[rtti::Enum]" ) {
    ...
    *src.value_as<TestEnum1>().ok() = TestEnum1::VAL_3;
    Object dst = static_type<TestEnum1>()->alloc_move_construct(src).ok();
    REQUIRE( dst.is_valid() == true );
    REQUIRE( dst.type().ok() == static_type<TestEnum1>() );
    REQUIRE( dst.size().ok() == sizeof(TestEnum1) );
    ...
    Object invalid_src;
    REQUIRE( static_type<TestEnum1>()->alloc_move_construct(invalid_src).err() == Type::ErrMoveConstruct::NOT_VALID_SOURCE );

    src = static_type<TestEnum2>()->alloc_construct().ok();
    REQUIRE( static_type<TestEnum1>()->alloc_move_construct(src).err() == Type::ErrMoveConstruct::INCORRECT_SOURCE_TYPE );
}

//*************************************************************************************************
TEST_CASE( "rtti::Enum::dealloc_destruct", "[rtti::Enum]" ) {
    Object src = static_type<TestEnum1>()->alloc_construct().ok();
    static_type<TestEnum1>()->dealloc_destruct(std::move(src));
...
TEST_CASE( "rtti::Enum::can_copy_construct", "[rtti::Enum]" ) {
    ...
    src = static_type<TestEnum1>()->alloc_construct().ok();
...
TEST_CASE( "rtti::Enum::copy_construct", "[rtti::Enum]" ) {
    char src_array[30];
    BufferRef buff_ref(reinterpret_cast<void*>(src_array), 30);
    Object src = static_type<TestEnum2>()->alloc_construct().ok();
...
TEST_CASE( "rtti::Enum::can_destruct", "[rtti::Enum]" ) {
    ...
    obj = static_type<TestEnum2>()->alloc_construct().ok();
    REQUIRE( static_type<TestEnum1>()->can_destruct(obj).err() == Type::ErrDestruct::INCORRECT_OBJECT_TYPE );
    
    obj = static_type<TestEnum1>()->alloc_construct().ok();
    REQUIRE( static_type<TestEnum1>()->can_destruct(obj).is_ok() == true );
    
    Object src = static_type<TestEnum1>()->alloc_construct().ok();
    REQUIRE( static_type<TestEnum1>()->can_destruct(src).is_ok() == true );

    Object invalid_src;
    REQUIRE( 
        static_type<TestEnum1>()->can_destruct(invalid_src).err() 
        == 
        Type::ErrDestruct::NOT_VALID_OBJECT );

    src = static_type<float>()->alloc_construct().ok();
    REQUIRE( 
        static_type<TestEnum1>()->can_destruct(src).err() 
        == 
...
TEST_CASE( "rtti::Enum::copy_assign", "[rtti::Enum]" ) {
    ...
    dst = static_type<TestEnum2>()->alloc_construct().ok();
    REQUIRE( 
        static_type<TestEnum1>()->copy_assign(dst, src).err() 
        == 
        Type::ErrCopy::INCORRECT_DESTINATION_OBJECT_TYPE );
    
    dst = static_type<TestEnum1>()->alloc_construct().ok();
    REQUIRE( 
        static_type<TestEnum1>()->copy_assign(dst, src).err() 
        == 
